...
REST API with RAG (Retrieval-Augmented Generation) for the BimBam Buy virtual assistant, a LATAM e-commerce platform. Built with **FastAPI**, **LangChain** and **Pinecone Serverless**, using **NVIDIA AI Endpoints** models.
...
├── ingest.py           # Offline ingestion pipeline (PDF → chunks → Pinecone)
├── rag.py              # RAG chain (retrieval + prompt + LLM)
├── store.py            # Pinecone client and retriever factory
...
This extracts text, splits it into chunks, generates embeddings, and upserts them to the Pinecone index (`PINECONE_INDEX_NAME`) under the configured namespace (`PINECONE_NAMESPACE`).
...
Returns Pinecone vector store and LLM status.

```json
// 200 — all good
{ "status": "healthy", "vectorstore": "connected", "llm": "available" }

// 503 — degraded
{ "status": "degraded", "vectorstore": "disconnected", "llm": "unavailable" }
```
...
| `PINECONE_API_KEY`       | `""`                                            | Pinecone Serverless API key (required) |
