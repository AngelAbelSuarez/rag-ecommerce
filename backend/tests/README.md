...
├── test_ingest.py      # Ingestion pipeline tests (PDF → chunks → Pinecone)
├── test_rag.py         # RAG chain tests (retrieval + prompt + LLM)
└── test_store.py       # Vector store tests (Pinecone client + retrievers)
```
...
| `test_store.py`  | 5  | Embeddings factory, Pinecone vector store creation, retriever defaults and overrides |
...
- Tests full `ingest()` with mocked Pinecone client
- Tests CLI `main()` entry point

### test_store.py

Tests the Pinecone client factory:

- Creates real `NVIDIAEmbeddings` instances (with fake API key)
- Mocks `PineconeVectorStore.from_existing_index` for the vector store creation
